...
\> This formulation of the variance with power spectrum tells us 
    ...
    where $\mathcal P_f(\omega)$ is big, we want $\mathcal P_S(\omega)$ to be small.
...
\>  An other type of anisotropic function we're interested in, 
...
\> \textit{Discussion}. The method has the advantage of being explicit. The blue noise
    generated isn't of highest quality. The sample for the double wedge spectrum 
    look terrible. Thus, it is probably not the good idea in order to produce anisotropic samples.
    Also, this algorithm has a major flaw, it doesn't scale well with higher number of point 
    inside a samples. 
...
\subsection{Modified slice optimal transport}
...
rectangle function.
    ...
    \begin{figure}[H]
        ...
        correspond to the means square error. The rectangle we integrate is $0.96$ unit 
        long and $0.12$ unit wide}\\
...
some particular direction, here are plot of means square error when $N$ increase:
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=110mm]{plot/integration_rec2_dw.png}\\
    \end{figure}  

\>  But, in reality, as the previous plot show, those improvement, are 
    only a constant time better than 
    blue noise. Which is good, but not as great that we initially want. In order to have achieve a new order of convergence, we would need to
    make the size of the interval depends on the number of sample. Intuitively, the power 
    spectrum of a sample is like having a set of frequency $\omega$ where $\mathcal P_S(\omega)$ is small, where $N$ is 
    the number of point in our samples of size $O(N)$. This is due to his strong link with PCF (pair correlation function). 
    Thus, we can't make this set grow in an constant infinite number of direction faster than $O(\sqrt N)$. And thus, the
    convergence rate can't be better than $O(N^{1.5})$[\cite{singh17convergence}]. 

\subsubsection{Cross in 2D}

\textit{Objective} Like introduce in section $2.2$, we also want to create sample
    distribution that have a power spectrum with a cross from. In order to do so, we 
...
\> \textit{Why does it matter ?} More generally, theoretically, if we compute the 
    isovalue of the power spectrum $\mathcal P_f$ of a function $f$, 
    and we generate sample that such that $\mathcal P_S$ has the same isovalue.
    We could improve the overall convergence rate.
...
\subsubsection{Attempt to 3D generalization}
    ...
    in the case of motion blur -it's a 3 dimensionnal space (XYT), we would like [\cite{singh17convergence}]
    \begin{itemize}
...
\subsubsection{Introduction to the idea}
    Let's look at good sampler to get inspiration.
    One things that characterize low discrepancy sampler, is that if we take the points
    within a subspace, it still has good properties.
    For instance, if take a sub rectangle of the 2D euclidean space. Points inside will still 
    have a great distribution (the 1D projection on the axis are good). Just like Dyadic Nets. 
